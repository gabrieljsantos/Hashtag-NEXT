...
### Iniciar o tabuleiro com as configurações
pygame.init()
tela = pygame.display.set_mode(tabu_size, 0, 32)
...
janela = Tk()
janela.title("STATUS do jogo")
janela["background"]= rgb_color(bg_color)
janela.geometry('200x200+1500+20')

def janela_score():
    score = Label (text=(str(win_n_x)+ 'x' +  str(win_n_o)),foreground=rgb_color((255, 255, 200)),background=rgb_color(bg_color),font='impact 80 bold' )
    score.place(x=10,y=5)
...
### Função que verifica se foi clicado no mause
vez_de_jogar = -1
def clicar():
    global jogadas
    global vez_de_jogar
...
### Função responsavel por desenhar as pessinha com base nas jogadas já realizadas
...
def veri_win(elementos=[]):
    ...
    ### Função que texta se existe alguma combinação de vitoria dentro das jogas
    for n in range (8) :
        run_true = TRUE
